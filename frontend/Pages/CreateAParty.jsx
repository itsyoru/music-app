...
function CreateOrJoinAParty() {
  ...
  const [partyVideos, setPartyVideos] = useState({});
      ...
      >
        ...
        >
          <form onSubmit={handleCreateParty}>
            <input
              type="text"
              value={partyNameToCreate}
              onChange={(e) => setPartyNameToCreate(e.target.value)}
          ...
          <span>
            Create a party to start listening, or join a preexisting one.
          </span>
          <form onSubmit={handleJoinParty}>
            <input
              type="text"
              value={partyNameToJoin}
              onChange={(e) => setPartyNameToJoin(e.target.value)}
              placeholder="Enter party name"
              required
              style={{
                marginRight: "10px",
          ...
          <ul style={{ listStyleType: "none", padding: "0" }}>
            {parties.map((partyName, index) => (
              ...
              >
                ...
                {partyVideos[partyName] && (
                  <>
                    <br />
                    Listening now...{" "}
                    <strong>{partyVideos[partyName].title}</strong>
                  </>
                )}
              </li>
            ))}
