...
function Profile() {
    ...
    const [username, setUsername] = useState(routeUsername || localStorage.getItem('username'));
    ...
    useEffect(() => {
        const fetchFavoriteAlbums = async () => {
            const response = await fetch(`http://localhost:5001/users/${username}/favoriteAlbums`);
            const data = await response.json();
    ...
    return (
        <>
            <Navbar />
        <div className='profile-details' style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <div style={{ width: '45%' }}>
